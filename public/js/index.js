const dom = {
    ...
    downloadPathSelect: document.getElementById("downloadPathSelect"),
    downloadPathSelectList: document.getElementById("downloadPathSelectList"),
...
const state = {
    path: safeGetLocalStorage('save-path') || 'nas', // 保存路径
    onlineSongs: [],
    searchResults: [],
...
function openPathSelectList(){
    if (state.pathSelectOpen) return
    state.pathSelectOpen = true;
    dom.downloadPathSelectList.classList.add('show')
}

function closePathSelectList(){
    if (!state.pathSelectOpen) return
    state.pathSelectOpen = false;
    dom.downloadPathSelectList.classList.remove('show')
}
...
function togglePathSelect(event) {
    event.preventDefault();
    event.stopPropagation();
    if (state.pathSelectOpen) {
        closePathSelectList();
    }else{
        openPathSelectList();
    }
...
function initPath(){
    switch (state.path){
        case 'nas':
            dom.pathNow.innerText = 'NAS';
            break;
        case 'browser':
            dom.pathNow.innerText = '浏览器';
            break;
        default:
            showNotification('保存地址不对，请重新选择', 'error');
            break
    }
}
function setupInteractions() {
    ...
    // 添加下载地址选择.
    if (dom.downloadPathSelect){
        dom.downloadPathSelect.addEventListener('click',togglePathSelect)
    }
    if (dom.downloadPathSelectList){
        dom.downloadPathSelectList.addEventListener('click',function (event) {
            safeSetLocalStorage('save-path', event.target.dataset.source)
            state.path = event.target.dataset.source
            initPath();
        });
    }
    ...
    try {
        ...
        if (audioData && audioData.url) {
            ...
            switch (state.path) {
                case 'nas':
                    // 先下载歌曲，后下载歌词.
                    const songName = (Array.isArray(song.artist) ? song.artist.join('-') : song.artist) + '/' + song.name;
                    const res = await fetch('/download?url=' + encodeURIComponent(downloadUrl) + '&name=' + songName + '.' + fileExtension, {});
                    if (res.ok) {
                        showNotification('歌曲下载成功', 'success');
                        const lrcUrl = API.getLyric({...song});
                        const lrcReq = await fetch(lrcUrl);
                        if (lrcReq.ok) {
                            const data = await lrcReq.json();
                            const lrcTxt = data.lyric || ''
                            if (lrcTxt) {
                                const lrcSaveReq = await fetch('/download', {
                                    method: 'POST',
                                    body: JSON.stringify({
                                        lrc: lrcTxt,
                                        name: songName + '.lrc'
                                ...
                                if (lrcSaveReq.ok) {
                                    showNotification('歌词下载成功', 'success')
                                } else {
                                    showNotification('歌词下载失败', 'error')
                                }
                            }
                        }
                    } else {
                        showNotification('歌曲下载失败', 'error')
                    }
                    break;
                case 'browser':
                    ...
                    break;
                default:
                    showNotification('下载位置不对，请重新选择', 'error');
                    break;
            }
